...
// Rele 1 - Pin 9 : Bremselys
// Rele 2 - Pin 8 : Vifte
// Rele 3 - Pin 5 : Pumpe
// Rele 4 - Pin 4 : AMS test relay
...
#define AMS_LIGHT_RELAY 4
#define PUMP_PIN 5
#define FAN_PIN 8
#define BRAKELIGHT_PIN 9
...
unsigned long brakelightDelay = 0;
...
int torque = 0;
unsigned long tempSendTime = 0, reset_timer = 0, r2dSoundStartTime, brakeImplausibilityTime;
...
float getBrakePressure()
{
  ...
  int pressure = bits * BITS_TO_BAR;
  if( pressure > 50000 || pressure < 0 ) {
    pressure = 0;
  }
  return pressure;
}
void setup()
{
  ...
  debugMessage.can_id = 0x0BB; // CANBUS ID
  debugMessage.can_dlc = 8;    // Length of the message
  //---CAN DATA START --- //
  debugMessage.data[0] = 0x00; // Commanded torque, first
  debugMessage.data[1] = 0x00; // Commanded torque, last
  debugMessage.data[2] = 0x00; // Commanded speed, first(not used)
  debugMessage.data[3] = 0x00; // Commanded speed, last(not used)
  debugMessage.data[4] = 0x01; // Commanded direction, 0 = reverse, 1 = forward
  debugMessage.data[5] = 0x00; // 5.0 Inverter enable(0 off, 1 on)
  debugMessage.data[6] = 0x00; // Not in use
  debugMessage.data[7] = 0x00; // Not in use
  //---CAN DATA END --- //
...
void loop()
{
  ...
  // Brakelight
  if (brakePressure >= 4 && brakelightDelay == 0)
  {
    brakelightDelay = millis();
    
  }else if (brakePressure >= 4 && millis() - brakelightDelay > 200){
    digitalWrite(BRAKELIGHT_PIN, HIGH);
    brakelightDelay = millis();
  }
  if (brakePressure <= 3 && millis() - brakelightDelay > 300)
  {
